...
\midrule
Total de estudos incluídos & 41 & 37 do protocolo mais 4 da busca suplementar \\
Natureza: framework & 37 & Predominância quase total \\
Natureza: arquitetura / plataforma / método & 1 / 1 / 2 & JulesWhite2023, YuCheng2023, JSauvola2024, Zabardast2025 \\
Com avaliação empírica & 28 & Algum grau de validação reivindicado \\
...
Estudo de caso & 6 & Ulfsnes2024, ZeeshanRasheed2024, Baron2025, Khan2026, Macedo2026, Tawosi2025 \\
Distribuição por ano & \multicolumn{2}{l}{2022:1, 2023:4, 2024:10, 2025:15, 2026:11} \\
Origem acadêmica ou técnica / industrial & 37 / 4 & Quatro estudos de indústria ou prática profissional \\
Confiança de extração alta & 35 & Média em seis estudos (inclui os 4 da busca suplementar) \\
\bottomrule
\end{tabularx}
\end{table}
...
\noindent\textbf{Avaliação de qualidade por categoria de fonte.} A Tabela~\ref{tab:qualidade} resume a avaliação de qualidade descrita no Método, com o checklist de quatro itens aplicado aos 41 estudos. Onze estudos (27\%) ficam na faixa de qualidade alta, vinte (49\%) na média e dez (24\%) na baixa. A maior parte do corpus vem de venue revisado por pares (29 estudos, 71\%), contra dez preprints arXiv (24\%) e dois registros técnicos não indexados (5\%). No nível dos itens, 28 estudos (68\%) reportam avaliação empírica e 22 (54\%) disponibilizam repositório público, mas apenas seis (15\%) trazem validação aplicada em estudo de caso ou projeto real, o que delimita o quanto a evidência atual sustenta uso em produção. A faixa baixa concentra-se em preprints e registros técnicos sem avaliação empírica registrada; seus achados são lidos com cautela na síntese e nenhum tema se apoia exclusivamente neles. A avaliação é conservadora por construção, pois deriva dos sinais registrados na extração e não de uma reanálise de texto completo estudo a estudo.

\begin{table}[h!]
\centering
\caption{Avaliação de qualidade por categoria de fonte dos 41 estudos. Itens derivados de sinais observáveis na matriz de extração; QA é a soma de QA1 a QA4 (0 a 4).}
\label{tab:qualidade}
\small
...
\midrule
Qualidade alta (QA $\geq$ 3) & 11 & 27\% \\
Qualidade média (QA = 2) & 20 & 49\% \\
Qualidade baixa (QA $\leq$ 1) & 10 & 24\% \\
\midrule
Venue revisado por pares (QA1) & 29 & 71\% \\
Preprint arXiv & 10 & 24\% \\
Registro técnico não indexado & 2 & 5\% \\
\midrule
Avaliação empírica reportada (QA2) & 28 & 68\% \\
Validação aplicada em estudo de caso (QA3) & 6 & 15\% \\
Artefato público para reprodutibilidade (QA4) & 22 & 54\% \\
\bottomrule
\end{tabularx}
\end{table}

\noindent\textbf{Evidência empírica reportada pelos estudos.} Uma segunda passada de extração sobre o texto completo recuperou, para cada estudo, o achado central e os resultados auto-reportados. O traço mais marcante é que esses resultados vivem em montagens heterogêneas e não comparáveis entre si, o que é, por si, um achado. Entre os números reportados: AutoDev atinge 91,5\% de Pass@1 em geração de código e 87,8\% em geração de testes no HumanEval, sem treino adicional \cite{MicheleTufano2024}; ASTRA encontra de 11\% a 66\% mais vulnerabilidades que técnicas existentes ao tratar segurança como red-teaming ativo \cite{XiangzheXu2025}; o desenvolvimento orientado por especificação relata até 50\% menos erros em código gerado por LLM quando alimentado com especificações refinadas \cite{DeepakBabuPiskala2026}; um blueprint de integração com governança multi-camadas relata, em estudo de caso industrial, ganho de vinte vezes na frequência de deployment e 40\% de redução no ciclo \cite{SKaruppuchamy2026}; a engenharia de requisitos consciente de risco é validada por modelagem de equações estruturais com 328 participantes \cite{Watfa2026}; e a engenharia de documentação reversa estrutura 517 afirmações com 97,1\% de confiança interna em uma migração de COBOL para Go \cite{Macedo2026}. No lado da adoção, o estudo de arquivos de contexto encontra uso em torno de 5\% em dez mil repositórios do GitHub \cite{SeyedmoeinMohsenimofidi2025}, e entre os estudos incorporados pela busca suplementar, AutoReview relata ganhos de 18,72\% em F1 de detecção, 27,75\% em precisão de localização e 14,82\% em BLEU de reparo de vulnerabilidades sobre os baselines \cite{Chen2025}. A ausência de um benchmark compartilhado entre essas montagens é justamente o que motiva a primeira lacuna da Agenda de Pesquisa.

\noindent\textbf{Seis temas indutivos.} A análise fez emergir seis temas que, juntos, cobrem os 41 estudos (cobertura de 100\%, acima do alvo de 80\%). Cada estudo pode pertencer a mais de um tema; a ordem reflete frequência decrescente.

\noindent\textbf{T1: orquestração multiagente e divisão de papéis.} Este é o tema mais populoso e o eixo arquitetônico dominante do corpus, respondendo diretamente à RQ1. A convergência central é que a estruturação do trabalho por papéis distintos, e não a escolha do modelo de linguagem, é o que diferencia os frameworks. ChatDev codifica uma cadeia de conversas com fases de design, codificação, teste e revisão, além de um mecanismo de redução comunicativa de alucinação \cite{ChenQian2023}. MetaGPT internaliza procedimentos operacionais padrão de engenharia de software, atribuindo a cada papel artefatos específicos: o gerente de produto gera requisitos, o arquiteto gera o design e as interfaces, o desenvolvedor implementa \cite{SiruiHong2023}. CodePori escala esse arranjo para agentes de gerência, arquitetura, estrutura, desenvolvimento, verificação e finalização \cite{ZeeshanRasheed2024}, e AutoDev e AutoCodeRover levam a coordenação de agentes a tarefas reais de melhoria de programas \cite{MicheleTufano2024,YuntongZhang2024}. As revisões de He et al. e a visão de Engenharia de Software 3.0 de Hassan et al. confirmam que a literatura já organiza esses sistemas por etapas do ciclo de vida \cite{JundaHe2025,AhmedEHassan2024}. O consenso, porém, é parcial: há tensão clara quanto ao grau de autonomia. Sapkota et al. contrastam explicitamente codificação por intuição e codificação agêntica \cite{RanjanSapkota2025}, e enquanto CodePori e MetaGPT se inclinam ao desenvolvimento autônomo de ponta a ponta, outros desenhos preservam pontos de decisão humana. Estudos recentes reforçam o eixo: ALMAS alinha seus agentes a papéis ágeis para atuar de ponta a ponta no ciclo \cite{Tawosi2025}, e Scrum-AI delega a geração de backlog e a gestão das cerimônias do Scrum a LLMs com integração ao Jira \cite{Erten2025}. A confiabilidade e o custo de coordenação aparecem como riscos recorrentes neste tema \cite{Dam2025,Alsegier2026,Merchant2025,Paduraru2026,SKaruppuchamy2026,Paiva2026,Akbar2025,LiyiCai2025,Anon2026,BESLEAGA2026}.
