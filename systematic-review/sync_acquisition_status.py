...
OUT = BASE / "snowballing" / "round-1" / "acquisition-summary.md"
RUN_LOCAL = "2026-05-28 12:03"
RUN_ISO = "2026-05-28T12:03:00Z"
...
def main() -> None:
    ...
    for entry in data:
        ...
        entry["acquisition"] = {
            ...
            "ext": record.get("ext"),
            "timestamp": RUN_LOCAL,
            "status": "downloaded" if record.get("success") else "manual_required",
        }
    ...
    snow = [
        ...
        if (entry.get("systematic_review") or {}).get("discovery") == "snowballing-1"
    ...
    downloaded = [entry for entry in snow if (entry.get("acquisition") or {}).get("success")]
    manual = [entry for entry in snow if not (entry.get("acquisition") or {}).get("success")]

    lines = [
        "# Acquisition Snowballing Round 1",
        "",
        f"> Executado em: {RUN_LOCAL}",
        "> Escopo: `snowballing-1` incluidos apos screening por titulo e abstract",
        "",
        "## Resultado",
        "",
        f"- Candidatos para texto completo: {len(snow)}",
        f"- Baixados ou ja presentes: {len(downloaded)}",
    ...
    print(json.dumps({
        "scope": "snowballing-1",
        "total": len(snow),
        "downloaded_or_present": len(downloaded),
