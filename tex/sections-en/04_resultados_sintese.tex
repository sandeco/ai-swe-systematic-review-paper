...
This section consolidates the findings of the 41 included studies (the 37 from the protocol plus four from the supplementary search). The synthesis follows a hybrid thematic approach: six themes were derived inductively from the structured fields of the extraction records (artifacts, roles, execution, validation, portability, risks, nature, and evidence) and then confronted with the six-dimension taxonomy proposed in the previous section, so as to test it against the evidence rather than assume it. The themes answer RQ1 through RQ4; RQ5, on gaps, is addressed in the Research Agenda. The strength of each study's evidence is weighted by a source-category quality assessment, described in the Method and summarized below (Table~\ref{tab:qualidade}).

\noindent\textbf{Quantitative overview.} Table~\ref{tab:quant} summarizes the composition of the corpus. The framework category dominates almost entirely (37 of the 41 studies), which empirically supports the thesis that the unit of work has shifted from the isolated prompt to the structured process. Empirical evaluation is claimed by 28 studies and 22 release a public repository, but ten are arXiv preprints not yet peer-reviewed, which calls for a qualified reading of the strength of the evidence (Figure~\ref{fig:evidence}). The temporal distribution shows steady acceleration of the field, from a single study in 2022 to a peak of fifteen in 2025 and eleven in 2026, with most of the corpus concentrated from 2024 onward (Figure~\ref{fig:year}). Venues are dispersed, with the ACM Transactions on Software Engineering and Methodology as a recurring outlet (five studies) followed by a long tail of conferences and journals with one or two studies each, consistent with an emerging field that still lacks a consolidated forum.

\begin{table}[h!]
\centering
\caption{Composition of the corpus of 41 included studies. Counts extracted from the extraction matrix.}
\label{tab:quant}
\small
...
\midrule
Total included studies & 41 & 37 from the protocol plus 4 from the supplementary search \\
Nature: framework & 37 & Near-total predominance \\
Nature: architecture / platform / method & 1 / 1 / 2 & JulesWhite2023, YuCheng2023, JSauvola2024, Zabardast2025 \\
With empirical evaluation & 28 & Some degree of validation claimed \\
...
Case study & 6 & Ulfsnes2024, ZeeshanRasheed2024, Baron2025, Khan2026, Macedo2026, Tawosi2025 \\
Distribution by year & \multicolumn{2}{l}{2022:1, 2023:4, 2024:10, 2025:15, 2026:11} \\
Academic or technical / industrial origin & 37 / 4 & Four studies from industry or professional practice \\
High extraction confidence & 35 & Medium in six studies (incl. the 4 supplementary) \\
\bottomrule
\end{tabularx}
\end{table}
...
\noindent\textbf{Quality assessment by source category.} Table~\ref{tab:qualidade} summarizes the quality assessment described in the Method, with the four-item checklist applied to the 41 studies. Eleven studies (27\%) fall in the high-quality band, twenty (49\%) in the medium band, and ten (24\%) in the low band. Most of the corpus comes from peer-reviewed venues (29 studies, 71\%), against ten arXiv preprints (24\%) and two non-indexed technical records (5\%). At the item level, 28 studies (68\%) report empirical evaluation and 22 (54\%) release a public repository, but only six (15\%) bring applied validation in a case study or real project, which bounds how far the current evidence supports production use. The low band concentrates preprints and technical records without recorded empirical evaluation; their findings are read cautiously in the synthesis and no theme rests exclusively on them. The assessment is conservative by construction, since it derives from the signals recorded in the extraction rather than from a full-text reappraisal of each study.

\begin{table}[h!]
\centering
\caption{Source-category quality assessment of the 41 studies. Items derived from observable signals in the extraction matrix; QA is the sum of QA1 to QA4 (0 to 4).}
\label{tab:qualidade}
\small
...
\midrule
High quality (QA $\geq$ 3) & 11 & 27\% \\
Medium quality (QA = 2) & 20 & 49\% \\
Low quality (QA $\leq$ 1) & 10 & 24\% \\
\midrule
Peer-reviewed venue (QA1) & 29 & 71\% \\
arXiv preprint & 10 & 24\% \\
Non-indexed technical record & 2 & 5\% \\
\midrule
Reported empirical evaluation (QA2) & 28 & 68\% \\
Applied validation in a case study (QA3) & 6 & 15\% \\
Public artifact for reproducibility (QA4) & 22 & 54\% \\
\bottomrule
\end{tabularx}
\end{table}

\noindent\textbf{Empirical results reported by the studies.} A second extraction pass over the full text recovered, for each study, the central finding and the self-reported results. The most striking trait is that these results live in heterogeneous, mutually non-comparable setups, which is itself a finding. Among the reported numbers: AutoDev reaches 91.5\% Pass@1 in code generation and 87.8\% in test generation on HumanEval, with no additional training \cite{MicheleTufano2024}; ASTRA finds 11\% to 66\% more vulnerabilities than existing techniques by treating security as active red-teaming \cite{XiangzheXu2025}; spec-driven development reports up to 50\% fewer errors in LLM-generated code when fed refined specifications \cite{DeepakBabuPiskala2026}; an integration blueprint with multi-layer governance reports, in an industrial case study, a twentyfold gain in deployment frequency and a 40\% cycle reduction \cite{SKaruppuchamy2026}; risk-aware requirements engineering is validated by structural equation modeling with 328 participants \cite{Watfa2026}; and reverse documentation engineering structures 517 claims with 97.1\% internal confidence in a COBOL-to-Go migration \cite{Macedo2026}. On the adoption side, the study of context files finds use in about 5\% of ten thousand GitHub repositories \cite{SeyedmoeinMohsenimofidi2025}, and among the studies added by the supplementary search, AutoReview reports gains of 18.72\% in detection F1, 27.75\% in localization precision, and 14.82\% in repair BLEU over the baselines \cite{Chen2025}. The absence of a shared benchmark across these setups is precisely what motivates the first gap of the Research Agenda.

\noindent\textbf{Six inductive themes.} The analysis surfaced six themes that, together, cover the 41 studies (100\% coverage, above the 80\% target). Each study may belong to more than one theme; the order reflects decreasing frequency.

\noindent\textbf{T1: multi-agent orchestration and role division.} This is the most populous theme and the dominant architectural axis of the corpus, answering RQ1 directly. The central convergence is that structuring the work by distinct roles, not the choice of language model, is what differentiates the frameworks. ChatDev encodes a chat chain with design, coding, testing, and review phases, plus a communicative dehallucination mechanism \cite{ChenQian2023}. MetaGPT internalizes standard operating procedures from software engineering, assigning specific artifacts to each role: the product manager generates requirements, the architect generates the design and interfaces, the developer implements \cite{SiruiHong2023}. CodePori scales that arrangement to agents for management, architecture, structure, development, verification, and finalization \cite{ZeeshanRasheed2024}, and AutoDev and AutoCodeRover bring agent coordination to real program-improvement tasks \cite{MicheleTufano2024,YuntongZhang2024}. He et al.'s reviews and Hassan et al.'s Software Engineering 3.0 vision confirm that the literature already organizes these systems by lifecycle stage \cite{JundaHe2025,AhmedEHassan2024}. Consensus, however, is partial: there is clear tension over the degree of autonomy. Sapkota et al. explicitly contrast vibe coding and agentic coding \cite{RanjanSapkota2025}, and while CodePori and MetaGPT lean toward end-to-end autonomous development, other designs preserve human decision points. Recent studies reinforce the axis: ALMAS aligns its agents with agile roles to operate end-to-end across the cycle \cite{Tawosi2025}, and Scrum-AI delegates backlog generation and the management of Scrum ceremonies to LLMs with Jira integration \cite{Erten2025}. Reliability and coordination cost appear as recurring risks in this theme \cite{Dam2025,Alsegier2026,Merchant2025,Paduraru2026,SKaruppuchamy2026,Paiva2026,Akbar2025,LiyiCai2025,Anon2026,BESLEAGA2026}.
